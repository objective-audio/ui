...
void node::attach_x_layout_guide(layout_value_guide &guide) {
    this->_x_canceller =
        guide.observe([this](float const &x) { this->_position->set_value(point{x, this->position().y}); }).sync();
...
void node::attach_y_layout_guide(layout_value_guide &guide) {
    this->_y_canceller =
        guide.observe([this](float const &y) { this->_position->set_value(point{this->position().x, y}); }).sync();
