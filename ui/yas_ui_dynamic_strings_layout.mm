//
//  yas_ui_dynamic_strings_layout.cpp
//
...
struct ui::dynamic_strings_layout::impl : base::impl {
    ...
    void prepare(ui::dynamic_strings_layout &strings_layout) {
        auto weak_strings_layout = to_weak(strings_layout);

        _collection_observer = _collection_layout.subject().make_observer(
            ui::collection_layout::method::actual_cell_count_changed, [weak_strings_layout](auto const &context) {
                if (auto strings_layout = weak_strings_layout.lock()) {
                    strings_layout.impl_ptr<impl>()->_update_layout();
                }
            });
   ...
   private:
        ...
        if (_args.font_atlas) {
            if (!_font_atlas_observer) {
                _font_atlas_observer = _args.font_atlas.subject().make_observer(
                    ...
                    [weak_strings_layout = to_weak(cast<ui::dynamic_strings_layout>())](auto const &context) {
                        if (auto strings_layout = weak_strings_layout.lock()) {
                            strings_layout.rect_plane().node().mesh().set_texture(
                                strings_layout.font_atlas().texture());
                            strings_layout.impl_ptr<impl>()->_update_layout();
                        }
                    });
    ...
    void _update_layout() {
        ...
        auto handler = [](ui::dynamic_strings_layout &strings_layout, std::size_t const idx, std::string const &word,
                          ui::region const &region) {
            auto &rect_plane_data = strings_layout.impl_ptr<impl>()->_rect_plane.data();

            if (idx < rect_plane_data.rect_count()) {
                auto const &font_atlas = strings_layout.font_atlas();
        ...
        auto strings_layout = cast<ui::dynamic_strings_layout>();
        ...
        for (auto const &idx : make_each(actual_cell_count)) {
            ...
            if (idx >= prev_rect_count) {
                cell_rect.set_value_changed_handler(
                    [idx, word, weak_strings_layout = to_weak(strings_layout), handler](auto const &context) {
                        if (auto strings_layout = weak_strings_layout.lock()) {
                            handler(strings_layout, idx, word, context.new_value);
                        }
                    });
            }

            handler(strings_layout, idx, word, cell_rect.region());
        }
    }
};

ui::dynamic_strings_layout::dynamic_strings_layout() : dynamic_strings_layout(args{}) {
}

ui::dynamic_strings_layout::dynamic_strings_layout(args args) : base(std::make_shared<impl>(std::move(args))) {
    impl_ptr<impl>()->prepare(*this);
}

ui::dynamic_strings_layout::dynamic_strings_layout(std::nullptr_t) : base(nullptr) {
}

ui::dynamic_strings_layout::~dynamic_strings_layout() = default;

void ui::dynamic_strings_layout::set_text(std::string text) {
    impl_ptr<impl>()->set_text(std::move(text));
}

void ui::dynamic_strings_layout::set_font_atlas(ui::font_atlas atlas) {
    impl_ptr<impl>()->set_font_atlas(std::move(atlas));
}

void ui::dynamic_strings_layout::set_line_height(float const line_height) {
    impl_ptr<impl>()->set_line_height(std::move(line_height));
}

void ui::dynamic_strings_layout::set_alignment(ui::layout_alignment const alignment) {
    impl_ptr<impl>()->_collection_layout.set_alignment(alignment);
}

std::string const &ui::dynamic_strings_layout::text() const {
    return impl_ptr<impl>()->text();
}

ui::font_atlas const &ui::dynamic_strings_layout::font_atlas() const {
    return impl_ptr<impl>()->font_atlas();
}

float ui::dynamic_strings_layout::line_height() const {
    return impl_ptr<impl>()->line_height();
}

ui::layout_alignment ui::dynamic_strings_layout::alignment() const {
    return impl_ptr<impl>()->_collection_layout.alignment();
}

ui::layout_guide_rect &ui::dynamic_strings_layout::frame_layout_guide_rect() {
    return impl_ptr<impl>()->_collection_layout.frame_layout_guide_rect();
}

ui::rect_plane &ui::dynamic_strings_layout::rect_plane() {
