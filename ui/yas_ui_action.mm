...
void ui::updatable_action::set_finish_handler(std::function<void(void)> handler) {
...
class ui::action::impl : public base::impl, public updatable_action::impl {
   ...
   public:
    static float convert(float const pos, action_curve const curve) {
        setup_curve();

        switch (curve) {
            case action_curve::ease_in:
                return _convert_value(_ease_in_curve, pos);
            case action_curve::ease_out:
                return _convert_value(_ease_out_curve, pos);
            case action_curve::ease_in_out:
                return _convert_value(_ease_in_out_curve, pos);
            default:
                break;
        }
    ...
    void update(time_point_t const &time) override {
        std::chrono::duration<double> const time_diff = time - start_time;
        auto value = time_diff.count() / duration;
        bool finished = false;
        ...
        value = convert(value, curve);

        update(value);

        if (finished && finish_handler) {
            finish_handler();
        }
    }

    void set_finish_handler(std::function<void(void)> &&handler) override {
        finish_handler = std::move(handler);
    }

    virtual void update(double const value) = 0;

    weak<ui::node> target{nullptr};
    time_point_t start_time = std::chrono::system_clock::now();
    double duration = 0.3;
    action_curve curve = action_curve::linear;
    std::function<void(void)> finish_handler;
};
...
double ui::action::duration() const {
    return impl_ptr<impl>()->duration;
}

ui::action_curve ui::action::curve() const {
    return impl_ptr<impl>()->curve;
}
...
void ui::action::set_duration(double const &duration) {
    impl_ptr<impl>()->duration = duration;
}

void ui::action::set_curve(action_curve const curve) {
    impl_ptr<impl>()->curve = curve;
}
...
struct ui::translate_action::impl : public ui::action::impl {
    void update(double const value) override {
...
struct ui::rotate_action::impl : public ui::action::impl {
...
struct ui::scale_action::impl : public ui::action::impl {
    void update(double const value) override {
        if (auto locked_target = target.lock()) {
...
struct ui::color_action::impl : public ui::action::impl {
    void update(double const value) override {
        if (auto locked_target = target.lock()) {
...
std::string yas::to_string(ui::action_curve const curve) {
    switch (curve) {
        case ui::action_curve::linear:
            return "linear";
        case ui::action_curve::ease_in:
            return "ease_in";
        case ui::action_curve::ease_out:
            return "ease_out";
        case ui::action_curve::ease_in_out:
            return "ease_in_out";
    }
}
