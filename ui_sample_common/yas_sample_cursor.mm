...
struct sample::cursor::impl {
    ...
    void prepare(sample::cursor_ptr const &cursor) {
        this->_renderer_observer =
            node->chain_renderer()
                .perform([weak_node = to_weak(this->node), event_observer = chaining::any_observer_ptr{nullptr}](
   ...
   private:
    void _setup_node() {
        ...
        this->node->add_sub_node(plane->node());
    }
    ...
    static chaining::any_observer_ptr _make_event_observer(ui::node_ptr const &node, ui::renderer_ptr const &renderer) {
        return renderer->event_manager()
            ...
            .perform([weak_node = to_weak(node),
                      weak_action = std::weak_ptr<ui::action>{}](ui::event_ptr const &event) mutable {
...
sample::cursor::cursor() : _impl(std::make_unique<impl>()) {
}
