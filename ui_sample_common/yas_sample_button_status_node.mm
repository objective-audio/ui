...
struct sample::button_status_node::impl : base::impl {
    ...
    impl(ui::font_atlas &&font_atlas) : _strings({.font_atlas = font_atlas, .max_word_count = 32}) {
        _strings.square_node().node().set_position(ui::point{0.0f, -7.0f});
        _strings.square_node().node().set_alpha(0.5f);
        _strings.set_pivot(ui::pivot::center);
        _strings.set_text("-----");
    }

    void set_status(sample::button_node::method const status) {
        _strings.set_text(to_string(status));
    }

   private:
    sample::button_node::method _status;
};
...
void sample::button_status_node::set_status(sample::button_node::method const status) {
