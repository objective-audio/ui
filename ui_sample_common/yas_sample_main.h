...
struct main {
   ...
   private:
    ...
    sample::touch_holder_ptr const _touch_holder =
        sample::touch_holder::make_shared(renderer->event_manager(), action_manager);
    sample::cursor_ptr const _cursor = sample::cursor::make_shared(renderer->event_manager(), action_manager);
    ...
    sample::cursor_over_planes_ptr const _cursor_over_planes =
        sample::cursor_over_planes::make_shared(renderer->event_manager(), action_manager, renderer->detector());
    ...
    sample::soft_keyboard_ptr const _soft_keyboard =
        sample::soft_keyboard::make_shared(_font_atlas, renderer->event_manager(), action_manager, renderer->detector(),
