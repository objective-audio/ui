...
struct sample::justified_points::impl {
    ui::rect_plane_ptr _rect_plane = ui::rect_plane::make_shared(sample::all_point_count);
    std::vector<ui::layout_guide_ptr> _x_layout_guides{sample::make_layout_guides(sample::x_point_count)};
    std::vector<ui::layout_guide_ptr> _y_layout_guides{sample::make_layout_guides(sample::y_point_count)};
    ...
    void prepare(sample::justified_points_ptr const &points) {
                ...
                .perform([weak_points = to_weak(points), x_layout = chaining::any_observer_ptr{nullptr},
                    ...
                    if (auto points = weak_points.lock()) {
                        if (renderer) {
                            ...
                            for (auto &guide : points->_impl->_x_layout_guides) {
                            ...
                            for (auto &guide : points->_impl->_y_layout_guides) {
        ...
        while (yas_each_next(y_each)) {
            ...
            this->_guide_observers.emplace_back(
                this->_y_layout_guides.at(idx)
                    ...
                    .perform([weak_plane, idx](float const &value) {
                        weak_plane.lock()->data()->set_rect_position(
                            {.origin = {-2.0f, value - 2.0f}, .size = {4.0f, 4.0f}}, idx + x_point_count);
                    })
                    .end());
        }
    }
...
sample::justified_points::justified_points() : _impl(std::make_unique<impl>()) {
}
...
void sample::justified_points::_prepare(justified_points_ptr const &shared) {
