...
void sample::main::setup(double const scale_factor) {
    ...
    root_node.add_sub_node(_bg_node.square_node().node());
    root_node.add_sub_node(_cursor_over_node.node());
    root_node.add_sub_node(_button_node.square_node().node());
    root_node.add_sub_node(_cursor_node.node());
    ...
    if (auto texture_result = ui::make_texture(renderer.device(), {1024, 1024}, scale_factor)) {
        ...
        root_node.add_sub_node(_text_node.strings_node().square_node().node());
        root_node.add_sub_node(_modifier_node.strings_node().square_node().node());
        root_node.add_sub_node(_button_status_node.strings_node().square_node().node());
    } else {
